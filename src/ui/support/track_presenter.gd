...
static func track_bounds(key: String) -> Array:
	if ["world_demand", "world_interest_rate", "gdp_gap", "exchange_rate", "current_account"].has(key):
		return [-5, 5]
	return [0, 10]
...
static func track_color(key: String, value: int, colors: Dictionary) -> Color:
	...
	if ["trade_openness", "global_coordination", "political_capital", "current_account"].has(key):
...
static func marker_count(key: String, value: int) -> int:
	...
	if ["trade_openness", "global_coordination", "political_capital"].has(key):
