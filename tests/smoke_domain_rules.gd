...
func _init() -> void:
	...
	for country in game.countries:
		...
		_assert(country.active_agenda.size() >= 4 and country.active_agenda.size() <= 7, "%s has a bounded active policy agenda" % country.display_name)
	...
	_assert(agenda_trim_game.countries[0].active_agenda.size() <= 7, "declared/support policy priority still respects the agenda size cap")
