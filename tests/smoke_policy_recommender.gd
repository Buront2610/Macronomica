...
func _init() -> void:
	...
	for country_index in range(game.countries.size()):
		...
		_assert(int(recommendation["policy_index"]) >= 0, "recommendation points to a policy menu card")
		_assert(CountryStateScript.WORKERS.has(String(recommendation["worker"])), "recommendation assigns a known worker")
		var card: Dictionary = game.countries[country_index].policy_menu[int(recommendation["policy_index"])]
